...
void AiGameMoveHandler::handle(const HttpRequest &req, HttpResponse *resp)
{
    ...
    { 
        json response = {
            ...
            {"message", e.what()}};
        std::string responseBody = response.dump();
        server_->packageResp(req.getVersion(), HttpResponse::HttpStatusCode::k500InternalServerError, "Internal Server Error", false, "application/json", responseBody.size(), responseBody, resp);
    }
}