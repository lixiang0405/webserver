    ...
    try
    {
        ...
        if (userId != -1)
        {
            ...
            else
            {
                ...
                resp->setStatusLine(req.getVersion(), HttpResponse::HttpStatusCode::k403Forbidden, "Forbidden");
                resp->setCloseConnection(true);
                resp->setContentType("application/json");
        ...
        else // 账号密码错误，请重新登录
        {
            ...
            resp->setStatusLine(req.getVersion(), HttpResponse::HttpStatusCode::k401Unauthorized, "Unauthorized");
            resp->setCloseConnection(false);
            resp->setContentType("application/json");
            resp->setContentLength(failureBody.size());
            resp->setBody(failureBody);
