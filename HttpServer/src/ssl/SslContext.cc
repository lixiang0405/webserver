...
bool SslContext::setupProtocol()
{
    // 设置 SSL/TLS 协议版本
    long options = SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3;
    long alloptions = SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_TLSv1_2 | SSL_OP_NO_TLSv1_3;
    switch (config_.getProtocolVersion())
    {
        //选择对应的版本，禁用其他版本
        case SSLVersion::TLS_1_0:
            options |= SSL_OP_NO_TLSv1 ^ alloptions;
            break;
        case SSLVersion::TLS_1_1:
            options |= SSL_OP_NO_TLSv1_1 ^ alloptions;
            break;
        case SSLVersion::TLS_1_2:
            options |= SSL_OP_NO_TLSv1_2 ^ alloptions;
            break;
        case SSLVersion::TLS_1_3:
            options |= SSL_OP_NO_TLSv1_3 ^ alloptions;
            break;
    }
    SSL_CTX_set_options(ctx_, options);
